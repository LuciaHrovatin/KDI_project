...
import json 
import pprint as pp 
dir = os.listdir(r'C:\Users\Anna Fetz\Desktop\Data_Science\third_semester\KDI_2021\KDI_PROJECT\EVENTS')

d = {os.path.join(r'C:\Users\Anna Fetz\Desktop\Data_Science\third_semester\KDI_2021\KDI_PROJECT\EVENTS',v): [] for v in dir}
...
def create_keys(dic) : 
    ...
    key_words = {'b': set(['biglietto', 'prezzo','ticket', 'prenotazione','gratuito','ingresso libero','partecipazione','gratis','libero','ingresso', 'entrata','gratuita']), 
                        ...
                        if ('info' in load) :  
                            for word in desc : 
                                ...
                                if (n in key_words['l']) : 
                                    ...
                                    load['is_offline'] = True 
                    for word in key_words['l'] :
                        if (word in ' '.join(load['indirizzo'].lower())) :
                            load['is_online'] = True
                        
                        if (word in load['title'].lower()) : 
                            load['is_online'] = True
                        else : 
                            load['is_offline'] = True
